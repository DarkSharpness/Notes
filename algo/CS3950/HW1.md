...
类似地, 我们将原图分为 $4$ 块, 分别递归求解. 我们定义 `divide(a,b,c,d)` 函数来求解 $[a,b] \times [c,d]$ 区域的局部最小值.

- 当初始 $n \le 5$ 的时候, 暴力.
- 否则, 先枚举边框 (即第一行第一列最后一行最后一列)的情况, 如果找到局部最小值, 则返回.
- 否则, 执行 `divide(1,n,1,n)` 函数.

`divide(a,b,c,d)` 的执行步骤如下:

- 如果 $b - a \le 3$ 或 $d - c \le 3$ 则直接枚举出当前区域的最小值.
- 否则, 令 $x = \dfrac {a + b} 2 , y = \dfrac {c + d} 2$. 求出第 $x$ 行, 第 $x + 1$ 行, 第 $y$ 列, 第 $y + 1$ 列这个区域的最小值. 如果这个最小值是局部最小, 那么已经找到. 否则由定义, 其周围至少存在一个不在这个区域内的点, 其比这个最小值还要小, 且其一定存在以下四个区域中的一个.
    1. $[a,x] \times [c,y]$
    2. $[a,x] \times [y+1,d]$
    3. $[x+1,b] \times [c,y]$
    4. $[x+1,b] \times [y+1,d]$
...
首先, 归纳证明在 `divide(a,b,c,d)` 执行过程中, 令 $p = b - a + 1, q = d - c + 1$, 不妨设 $p \le q$ (否则简单交换行列后处理即可), 那么有 $q \le 2p + 2$. 
...
$$
\begin{aligned}
q' &\le \lceil \frac q 2 \rceil \\
& \le \lceil \frac {2p + 2} 2 \rceil \\
& = p + 1 \\
& \le 2 \lfloor \frac p 2 \rfloor + 1 + 1 \\
& \le 2p' + 2 \\
\end{aligned}
$$

其次, 归纳证明在 `divide(a,b,c,d)` 执行过程中, $[a,b] \times [c,d]$ 区域内的最小值一定不在边框上. 即在内部.

- 初始调用时, 由于我们检测了边框上最小值不是局部极小值, 因此满足.
- 递归调用时, 由于其我们发现了内部交错的两行两列的最小值, 其不是局部极小值, 所以我们接下来调用 `divide` 选中的区域内部一定包含一个比这个最小值更小的值, 所以接下来的 `divide` 也满足性质.

最后, 我们证明这个算法一定会终止, 且终止状态一定正确.

- 首先, 每次递归时若 $p,q \ge 2$ 则 , $p' \le \lceil \frac p 2 \rceil \le p - 1$, 因此, 其一定会递归到 $p \lt 3$ 或 $q \lt 3$ 的情况, 即不会死递归.
- 对于终止状态, 注意到我们已经归纳证明了区域内的最小值一定不在边框上, 所以我们暴力求出最小值后, 其一定在非边框的内部. 由定义, 其一定是一个局部最小值. 正确性保证.

<!-- 至于时间复杂度, 留给读者作为习题. -->

至于时间复杂度, 首先, 每次 `divide` 里面要暴力求出两行两列的最小值, 复杂度为 $C(p + q)$ 其中 $C$ 为常数. 然后, 递归调用的时候, 每次递归后 $p$ 或 $q$ 至少减半, 即有:

$$ 
T(p,q) = T(\lceil \frac p 2 \rceil, \lceil \frac q 2 \rceil) + C(p + q)
$$

特别地, 当 $p \le 4$ 或 $q \le 4$ 时, 递归终止, 有 $T(p,q) = \mathcal{O}(1)$ (这是因为假如 $p \le q$, 由归纳证明, $q \le 2p + 2$, 所以 $p \le 4$ 时, $q \le 10$, 因此常数时间可以暴力求出极值). 

最终, 根据主定理, $T(p,q) = \mathcal{O}(p + q)$.

而初始 $p = q = n$, 因此 $T(n) = \mathcal{O}(n)$.

## c

我们先引入一个新的做法:

对于一个 $m \times n$ 的矩阵, 我们采用新的算法 `divide(a,b)` 表示分治求出二维区域 $[a,b] \times [1,m]$ 的局部最小值. 我们不妨假设 $m \le n$
...
以上这个做法当然称不上好, 但是, 我们可以把这个算法和前面的算法结合.

不妨设初始 $n \ge m$. 如果 $n \gt 2m + 2$, 则执行上面这个算法, 直到满足, $n' \le 2m + 2$. 此时, 选中区域满足 $c$ 中算法的性质, 即最值一定不在最左边或最右边的一列. 此时, 我们依然需要类似 $b$, 额外验证最上面一行和最下面一行的最小值是否是局部最小值. 如果不是, 那么此时, 我们已经满足了 $b$ 中 `divide` 所要求的先决条件, 即最小值不在边框上, 且 $n' \ke 2m + 2n$ 且 $n' \ge m$. 此时, 我们可以直接执行 $b$ 中的 `divide` 算法即可. 正确性自然可以保证.

下面是时间复杂度分析:

- 当 $n \gt 2m + 2$ 时, 我们执行了 $c$ 中的算法, 直到 $m \le n' \le 2m + 2$. 在这一步, 时间复杂度为 $T_1(m,n) = \mathcal{O}(m \log \frac {n} {n'})$.
- 之后, 我们调用了 $b$ 中的算法, 时间复杂度为 $T_2(m,n) = \mathcal{O}(m + n')$.

因此, 总复杂度为: (假设为 $m\le n$)

$$
\begin{aligned}
T(m,n) &= T_1(m,n) + T_2(m,n)\\
&= \mathcal{O}(m \log \frac {n} {n'}) + \mathcal{O}(m + n') \\
&= \mathcal{O}(m \log \frac {n} {m} + m)
\end{aligned}
$$

(特别地,为了防止出现 $n = m$ 时候, $\log$ 项为 $0$, 额外加了一项 $m$)

当 $m = 1$ 时, 即为一维的情况: $T(n) = \mathcal{O}(\log n)$.

当 $m = n$ 时, 即为方阵的情况: $T(n) = \mathcal{O}(n)$.
...
Get much help from [LYF](https://github.com/time-inter-space) in T4#b.
